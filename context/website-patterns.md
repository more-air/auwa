...
**Keep the persistent compositor layer on reveal wrappers — promote bordered descendants to their own layer.** A FadeIn wrapper at `transform: none` once visible demotes the GPU layer at transition end. Safari re-rasterises the element at integer pixels, producing a subtle "settle" shift (the unpolished feel the More Air site doesn't have — More Air's `.c-drift-into-place` keeps `translateZ(0)` in its end state). Fix: FadeIn and TextReveal now hold `translate3d(0, 0, 0)` at rest (mirroring More Air). The bordered-descendant clip this used to cause on iOS (e.g. primary CtaLink under the homepage intro) is solved by giving the CtaLink its own compositor layer via `transform: translateZ(0)` on the outer Link, so its border is rasterised independently of any ancestor's layer bounds.
