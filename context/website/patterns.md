...
**Homepage routes:**
- Root `/` — Live homepage. EditorialFrames (desktop ≥md) + PillarParade (mobile <md) as the "What we make." module, via a `hidden md:block` / `md:hidden` pair. Three frames (Book / Store / Journal) since 5 Aug 2026.

**"What we make." module (replaces the scroll-driven flipbook on the live homepage):**
...
- "What we make." module (EditorialFrames desktop / PillarParade mobile), three frames: Book / Store / Journal.
...
- `EntranceLoader` — **PARKED 22 Jul 2026** (commented out in layout.tsx, component file kept intact). Once-per-session あうわ reveal on a warm `#f8f2e5` overlay: chars rise in with 180ms stagger, reverse-stagger exit (わ→う→あ), overlay fades, unmount. Respects prefers-reduced-motion and sessionStorage flag. Removed because it held up every cold visitor with ~3s of opaque splash before the page appeared — no payoff for an English audience, and the Awwwards reason it was added for has passed. Worst for paid mobile traffic: ad clicks open a fresh in-app webview each time, so the sessionStorage guard never applies and the full splash played on every visit. Nothing reads its storage flag, so removal is self-contained; to restore, uncomment the import + render in layout.tsx.
...
**`TextReveal` emits an unconditional trailing nbsp after every word — including the last one.** The per-word span renders `{word}{" "}` rather than the previous `{word}{i < words.length - 1 ? " " : ""}`. Reason: stacked-block H1s (two `<TextReveal as="span" className="block">` calls inside one `<h1>`, used on `/about` and the 404) had no whitespace between the two blocks in rendered text content, because JSX collapses inter-element whitespace AND the last word of each TextReveal previously emitted no trailing character. innerText of the parent `<h1>` came out as "The architectureof Kokoro." — broken for screen readers, broken for SEO crawlers, broken for any code that reads `textContent`. Always-emit guarantees a word boundary across every stacked use without changing single-line wrap behaviour (nbsp prevents mid-heading break the same as before; a trailing nbsp at the end of any heading is invisible). **DO NOT "tidy" this back to the conditional form** without first verifying that `/about` and 404 H1 innerText still contain "The architecture of Kokoro." and "Lost the path." with proper spacing.
...
**`HeroVideo` mobile MQ uses `not (min-width: 64rem)`, not `max-width: 1023px`.** Tailwind's `lg` breakpoint is `min-width: 64rem` (1024px at the default 16px root). The naive complementary form `(max-width: 1023px)` looks fine at integer pixel widths, but at sub-pixel viewport widths on retina (e.g. 1023.5px) both `max-width: 1023px` and `min-width: 1024px` evaluate false — leaving a 0.5px no-man's-land where the CSS `lg:hidden` visible element diverges from the JS-selected video. Invert the desktop query with `not (min-width: 64rem)` and the boundary stays perfectly complementary across every viewport width. General principle: when a JS `matchMedia` mirrors a Tailwind responsive class, mirror the *exact* same min-width assertion (with `not`), don't approximate with `max-width: N-1px`.
...
**`PageTransition` `revealReady` gates on actual page paint via timer + double rAF, not a fixed 380ms timer alone.** After pathname change: `setTimeout(80 + REVEAL_DELAY_MS, () => requestAnimationFrame(() => requestAnimationFrame(() => setRevealReady(true))))`. The setTimeout gives the panel time to exit + the deliberate blank beat; the double rAF then waits for the browser to paint at least once past that point. Why double rAF: a single rAF runs *before* the next paint; the second rAF guarantees we're past at least one commit. On slow networks or heavy client-side renders the new page may still be mounting at the 380ms mark — without the paint gate, per-element reveals fire against a half-mounted DOM and the user sees a partial cascade. **Keep both the timer AND the double rAF.** The timer alone is what we had; the rAF alone would fire before the blank beat completes (the brand reads "snap, beat, arrive" — the beat is the timer's job). Cleanup must `cancelAnimationFrame` both rAF ids in the effect teardown.
...
**Viewport-alternative components must NOT carry their own canonical heading.** The "What we make." module renders `<EditorialFrames>` on `md+` and `<PillarParade>` on `<md`, gated by `hidden md:block` / `md:hidden`. Both used to own an `<h2>` (then "Four ways in.", now "What we make.") inside their own section wrapper — `display:none` hid one from sight at any given viewport, but the SSR HTML still contained both H2s, and Googlebot indexed both as duplicates of the same heading. Fix: hoist the single canonical `<h2>` to the homepage section that wraps both variants; each component now returns only its inner content (`<EditorialFrames>` keeps its `<section ref bg-surface>` for refs + hover handlers, `<PillarParade>` keeps its `<section ref>` for the scroller IO — neither carries the `space-section` padding any more because the wrapping section provides it once). General principle: any pair of viewport-alternative components with shared semantic intent — heading, landmark, region — should declare the semantic once at the page level and render only style/layout differences in the per-viewport siblings. **Reintroducing an `<h2>` inside either component is drift, even if it "tidies the component to be self-contained" — the duplicate SSR heading returns.**
...
**Audio `preload` is `"metadata"`, not `"auto"`.** `SoundToggle` constructs its `Audio` element on mount with `a.preload = "metadata"` (then `a.load()` to kick the fetch). Trade-off: with `"auto"`, the full ~3MB drift.mp3 downloaded on every initial page load — solved the Android Chrome "first tap rejects play()" issue, but cost the entire mobile Lighthouse performance budget (LCP +2-3s) for an interaction most visitors never trigger. With `"metadata"`, only a few KB of audio headers download eagerly so the element is fully initialised before any click; the full buffer fetches when `play()` is called by the user gesture. First tap still works on Android Chrome (the element is ready, play() resolves) — just with a brief buffer-fill delay on the *first* activation. The brand voice ("quiet, unhurried") accommodates a small delay; the alternative — eagerly downloading 3MB of audio nobody asked for — does not. **DO NOT switch back to `"auto"`** without confirming the Lighthouse mobile budget can absorb it.
