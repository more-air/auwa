...
**Viewport-alternative components must NOT carry their own canonical heading.** The four-pillar module renders `<EditorialFrames>` on `md+` and `<PillarParade>` on `<md`, gated by `hidden md:block` / `md:hidden`. Both used to own an `<h2>Four ways in.</h2>` inside their own section wrapper — `display:none` hid one from sight at any given viewport, but the SSR HTML still contained both H2s, and Googlebot indexed both as duplicates of the same heading. Fix: hoist the single canonical `<h2>` to the homepage section that wraps both variants; each component now returns only its inner content (`<EditorialFrames>` keeps its `<section ref bg-surface>` for refs + hover handlers, `<PillarParade>` keeps its `<section ref>` for the scroller IO — neither carries the `space-section` padding any more because the wrapping section provides it once). General principle: any pair of viewport-alternative components with shared semantic intent — heading, landmark, region — should declare the semantic once at the page level and render only style/layout differences in the per-viewport siblings. **Reintroducing an `<h2>` inside either component is drift, even if it "tidies the component to be self-contained" — the duplicate SSR heading returns.**
