// Dev utility: render the single newsletter template and either write it to
...
// Broadcast path in /api/single/send, so it's safe to run while iterating on
// format, and it never touches the real audience.
//
//   Usage:
//     # send assembled content to one address:
//     npx tsx --tsconfig scripts/tsconfig.json scripts/send-single-test.tsx <to> [props.json]
//     # render to HTML only, no send (for local preview):
//     RENDER_ONLY=/path/out.html npx tsx --tsconfig scripts/tsconfig.json scripts/send-single-test.tsx <to> [props.json]
//
...
// its fields (preview, intro, season, updates) are passed to SingleEmail.
import { readFileSync, writeFileSync } from "node:fs";
import { render } from "@react-email/render";
import { Resend } from "resend";
import SingleEmail from "../src/emails/single";
...
async function main() {
  ...
  let subject = "A new piece from Auwa (sample)";
  ...
  const html = await render(SingleEmail(props));

  const renderOnly = process.env.RENDER_ONLY;
  if (renderOnly) {
    writeFileSync(renderOnly, html, "utf8");
    console.log(`Rendered single HTML to ${renderOnly} (no send)`);
    return;
  }
  ...
  console.log(`Sent single test to ${to}, id ${data?.id}`);
}
